...
`auto_switch` 默认为 `false`，因此现有主题配置会保持手动行为。如果省略 `light_name` 或 `dark_name`，且配置的 `name` 存在对应的内置姊妹主题，Herdr 会使用该主题，例如 `tokyo-night`/`tokyo-night-day`、`gruvbox`/`gruvbox-light` 或 `everforest`/`everforest-light`。在设置中手动选择主题会禁用 `auto_switch`。
