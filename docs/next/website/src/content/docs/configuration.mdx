...
`auto_switch` defaults to `false`, so existing theme configs keep manual behavior. If `light_name` or `dark_name` is omitted, Herdr uses the matching built-in sibling for the configured `name` when one exists, such as `tokyo-night`/`tokyo-night-day`, `gruvbox`/`gruvbox-light`, or `everforest`/`everforest-light`. Manual theme selection in Settings disables `auto_switch`.
